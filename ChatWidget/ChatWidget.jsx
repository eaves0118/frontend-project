          ...
          <div className={styles.container}>
            <header className={styles.header}>
              <div className={styles.headerLeft}>
                ...
                <div className={styles.headerSubtitle}>
                  {doctors[activeDoctor].name} — {doctors[activeDoctor].specialty}
                </div>
              </div>
              ...
              <div className={styles.headerRight}>
                ...
                <button className={styles.minimizeBtn} onClick={minimize} aria-label="Thu nhỏ">
                  ─
                </button>
            ...
            <div className={styles.body}>
              <aside className={styles.sidebar}>
                <input className={styles.search} placeholder="Tìm kiếm bác sĩ..." />

                <div className={styles.list}>
                  {doctors.map((doc, idx) => (
                    <div
                      key={doc.id}
                      className={`${styles.listItem} ${activeDoctor === idx ? styles.active : ""}`}
                    ...
                    >
                      <div className={styles.avatar}>
                        ...
                        {doc.avatar ? (
                          ...
                          <span>{(doc.name || "BS").split(" ").slice(-1)[0].slice(0,2)}</span>
                        )}
              ...
              <section className={styles.chatPanel}>
                <div className={styles.messages}>
                  {messages[activeDoctor].length === 0 && (
                    <div className={styles.welcome}>
                      Xin chào! Tôi là {doctors[activeDoctor].name}. Tôi có thể giúp gì cho bạn?
                    </div>
                  )}
                  ...
                  {messages[activeDoctor].map((m, i) => (
                    <div
                      ...
                      className={`${styles.messageRow} ${
                        m.from === "me" ? styles.msgMe : m.from === "doctor" ? styles.msgDoctor : styles.msgSystem
                      }`}
                    >
                      {m.from !== "me" && m.from !== "system" && (
                        <img src={m.avatar} alt="avatar" className={styles.msgAvatar} />
                      )}
                ...
                <div className={styles.footer}>
                  ...
                  />
                  <button className={styles.sendBtn} onClick={sendMessage}>Gửi</button>
                </div>
              </section>
            </div>
          </div>
